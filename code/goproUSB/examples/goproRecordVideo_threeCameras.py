...
with concurrent.futures.ThreadPoolExecutor(max_workers=3) as pool:
    results = pool.map(shootVideo, [cam1,cam2,cam3])

#wait for the camera to finish processing:
while cam1.camBusy() or cam2.camBusy() or cam3.camBusy():
    continue
while cam1.encodingActive() or cam2.encodingActive() or cam3.encodingActive():
    continue


ml1 = cam1.getMediaList()
ml2 = cam2.getMediaList()
ml3 = cam3.getMediaList()
...
cam3.mediaDownloadLast(f'{fname}3_{fileidx:03}')
