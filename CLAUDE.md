...
- **Audio sync** (`audio_sync.py`): Clap-onset detection + frame-accurate ffmpeg trim (re-encodes — stream copy can't cut sub-GOP; GoPro keyframes ~1s apart). Runs automatically after recording. `evaluate_sync_acceptance()` gates quality (2 claps + half-frame consistency + <200ms inter-camera offset); on failure a shared red popup appears and the trial is discarded (Recording tab) or extrinsic calibration is aborted before the expensive compute (Calibration tab). See `docs/audio_sync_spec.md`.
